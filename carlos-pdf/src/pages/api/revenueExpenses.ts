  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (owner_query_result.length>0) {
        ...
        const sesssion_query_result = await queryMaker("sessions", {owner_id:owner_id.toString()})
        
        if (sesssion_query_result.length>0 && sesssion_query_result[0].sessionId == sessionId) {
            ...
            for (let index = 0; index < cars_id_array.length; index++) {
                ...
                const car_info_array = await queryMaker("cars", {_id:new ObjectId(car_id)})
                const car_revenue_array= await queryMaker("Revenue_Expenses", {car_id:car_id})
                let total_Revenue=0
